...
/**
 * Every chain Mayan touches, not just the ones big enough to draw.
 *
 * The Sankey necessarily truncates — twelve ribbons is the limit of what stays
 * readable — which made smaller destinations like Monad and Sui disappear from
 * the page entirely even though the app tracks them. This table is the complete
 * picture, and the reason nothing is hidden behind a top-N cut.
 */
export function RoutingByChain({
  rows,
  idleChains,
  className,
}: {
  rows: readonly ChainRouting[];
  idleChains: readonly string[];
  className?: string;
}) {
  if (rows.length === 0) return null;
  const busiest = Math.max(
    ...rows.map((row) => row.inflowUsd + row.outflowUsd),
    1,
  );
    ...
    <div className={className}>
        ...
        <table className="w-full min-w-[560px] text-[12px]">
          <thead>
            <tr className="border-hairline text-ink-muted border-b text-[10.5px] tracking-wide uppercase">
              ...
              <th scope="col" className="w-[34%] py-2 text-left font-medium">
                Share of routing
              </th>
          ...
          <tbody>
            {rows.map((row) => {
              const total = row.inflowUsd + row.outflowUsd;
              return (
                <tr
                  key={row.chain}
                ...
                >
                  <th
                    scope="row"
                    className="text-ink py-2 text-left font-medium whitespace-nowrap"
                  >
                    {row.chain}
                  </th>
                  <td className="py-2 pr-4">
                    <span className="flex items-center gap-2">
                      <span className="bg-grid relative h-[6px] flex-1 overflow-hidden rounded-full">
                        <span
                          className="absolute inset-y-0 left-0 rounded-full"
                          style={{
                            width: `${(total / busiest) * 100}%`,
                            background: "var(--color-series-1)",
                            opacity: 0.45 + (total / busiest) * 0.4,
                          }}
                        />
                      </span>
                      <span className="tnum text-ink-muted w-9 text-right text-[11px]">
                        {(row.share * 100).toFixed(1)}%
                      </span>
                    </span>
                  </td>
                  <td className="tnum text-ink-secondary py-2 text-right">
                    {formatUsd(row.inflowUsd)}
                  </td>
                  <td className="tnum text-ink-secondary py-2 text-right">
                    {formatUsd(row.outflowUsd)}
                  </td>
                  <td
      ...
      {idleChains.length > 0 && (
        <p className="text-ink-faint mt-3 text-[11px] leading-relaxed">
          Mayan also routes to {idleChains.join(", ")}, which saw no volume in
          this window.
        </p>
      )}
    </div>
